...
import itertools
from bsz.utils import bsplitz_method, fast_skewness_improvements, fast_variance_improvements
from bsz.cube_to_zonotope import enumerate_all_points


def report_regression_comparison(mean_encoding, generators, d, measure=fast_skewness_improvements, measure_name='skewness'):
    ...
    print('best %s improvement from greedy enumerating' %
          measure_name, np.max(new_res))
    print('best %s improvement from smart enumerating' %
          measure_name, np.max(similar_res))
...
def main():
    ...
    hx = OneHotEncoder(sparse=False, categories='auto').fit_transform(
        x[:, np.newaxis])
