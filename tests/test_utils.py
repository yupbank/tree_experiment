...
from bsz.utils import fast_gini_improvements, fast_variance_improvements, fast_skewness_improvements,\
    variance_improvement, skewness_improvement, _regression_summary_vector, gini_improvement, _classification_summary_vector,\
...
def test_fast_gini_the_same_with_gini(classification_target_split):
    ...
    data = preprocessing.OneHotEncoder(
        sparse=False,
        categories='auto').fit_transform(y[:, np.newaxis])
